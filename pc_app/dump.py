...
# def recommend_gpu(gpu_id):
#     gpus = GPU.objects.all()
#     gpu_data = np.array([[gpu.price, gpu.chipset, gpu.memory, gpu.core_clock, gpu.boost_clock, gpu.color, gpu.length] for gpu in gpus])
#     nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(gpu_data)
#     distances, indices = nbrs.kneighbors(gpu_data[gpu_id])
#     recommended_gpus = [gpus[i] for i in indices[0]]
#     return recommended_gpus
# #

# Fetch GPU data from the database
gpu_data = GPU.objects.all().values('price', 'memory', 'core_clock', 'boost_clock', 'length')

# Create a numpy array from the data
gpu_matrix = np.array([list(gpu.values()) for gpu in gpu_data])

# Calculate cosine similarity
cosine_sim = cosine_similarity(gpu_matrix, gpu_matrix)

# Initialize KNN model
knn = NearestNeighbors(n_neighbors=5, metric='cosine')
knn.fit(gpu_matrix)

def recommend_gpu(request):
    if request.method == 'POST':
        # Extract user's GPU features from the form
        selected_features = ['price', 'memory', 'core_clock', 'boost_clock', 'length']
        user_features = [request.POST.get(feature) for feature in selected_features]
        # user_features = [request.POST.get('price'), request.POST.get('memory'),
        #                  request.POST.get('core_clock'), request.POST.get('boost_clock'),
        #                  request.POST.get('length')]
        
        # Retrieve FavouritedPC objects and include their attributes
        if not any(user_features):
            # If the form is empty, use the attributes from FavouritedPC
            user_features = []

            favourited_pc_list = FavouritedPC.objects.all()
            
            for pc in favourited_pc_list:
        ...
        recommended_gpus = [GPU.objects.get(pk=index) for index in indices[0]]

        # Serialize GPU data for JSON response
        recommendations = [{'name': gpu.name, 'price': gpu.price} for gpu in recommended_gpus]
